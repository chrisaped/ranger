...
export default function Search({
  ...
  const isAnAsset = (searchParams) => {
    if (searchParams in tradeableAssets) return true;
    return false;
  };
  ...
  const isDisabled =
    !searchParams || watchlist.includes(searchParams) || isAPosition();

  const handleKeyPress = (e) => {
    if (e.key === "Enter" && searchParams && !isDisabled) getStockQuote();
  };
